...
var setupElements = function(data) {
  ...
  stripe = Stripe(data.publishableKey, {
    stripeAccount: paymentIntentData.account
  });
...
const updatePaymentIntent = (account, shouldSetupElements = false) => {
  ...
  // The account will be used as the Stripe-Account header when creating the
...
fetch("https://app.gobizgrow.com/recent-accounts", {
    method: "GET",
    headers: {
      "Content-Type": "application/json"
    }
  ...
  .then(data => {
    ...
    var cardPaymentsAccounts = accounts.filter((acct) => !!acct?.capabilities?.card_payments);
    if (!cardPaymentsAccounts.length) {
      document.querySelector("#no-card-payments-section").classList.remove("hidden");
      return;
    }

    var enabledAccounts = accounts.filter((acct) => acct?.capabilities?.card_payments === 'active');
